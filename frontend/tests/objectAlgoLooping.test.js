...
function searchByLoopingTest(searchIn, searchFor, isIncludingSearch){
    for (let i = 0; i < searchIn.length; i++) {
        if (typeof searchIn[i] === 'number'){ searchIn[i] = searchIn[i].toString()}
        if (typeof searchIn[i] === 'string') {
            if (searchIn[i].toLowerCase().includes(searchFor.toLowerCase())) {
                isIncludingSearch.push(searchIn[i])
            }
        } else if (Array.isArray(searchIn[i])) {
            for (let j = 0; j < searchIn[i].length; j++) {
                const values = Object.values(searchIn[i][j])
                for (let k = 0; k < values.length; k++) {
                    if (typeof values[k] === 'string') {
                        if (values[k].toLowerCase().includes(searchFor.toLowerCase())) {
                            isIncludingSearch.push(values[k])
                        }
                    }
                }
            }
