    ...
    createFilterChildren(noDuplicateFilters, filter, tagsArray, filterType, selectedTagsArrayName) {
        for (let tag of noDuplicateFilters) {
            if (!(tagsArray.includes(tag.replace(tag[0], tag[0].toUpperCase())))) {
                filter.appendChild(factory.createDOMElement('a', { role: 'option', class: `filter-item dropdown-filter-item__${filterType} text-white`, href: '#', 'data-group-name': `${selectedTagsArrayName}`, "data-name": `${filterType}` }, `${tag.replace(tag[0], tag[0].toUpperCase())}`));
            }
        }
    }
